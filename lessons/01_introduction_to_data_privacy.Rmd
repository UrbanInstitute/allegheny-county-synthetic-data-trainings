...
-   In addition there are many version of the data we should define:
    ...
    -   **Confidential dataset** (aka gold standarad/actual data) - cleaned version (cleaned census microdata).
...
it's important that **everyone sign up for an Rstudio Cloud account and join the `allegheny-count-data-privacy-trainings` space** so that you can follow along with our code exercises. You can do that by using this link: [\<https://rstudio.cloud/spaces/\>](https://rstudio.cloud/spaces/){.uri}. You will know you've successfully joined the class space when you see the following in the top left of your Rstudio Cloud account:

![](images/rstudio_cloud_space.png){width="209"}

We will be using Rstudio Cloud from Session 2 onwards to do code exercises in R (or if you wish Python). You can find Intro to R and Rstudio resources [here](https://rstudio.cloud/learn/primers). You can also come to our office hours to get more information and training on R and Rstudio Cloud.

<br>

# Optional HW

You can access the optional HW [here](). We promise this won't take more than 5 minutes and will give us valuable feedback on how we're doing.
